...
import { PAIN_MAX, painAreas, type PainKey } from "../data/painAreas";
...
export function ProfilePage() {
  ...
  const [painDraft, setPainDraft] = useState<Partial<Record<PainKey, number>>>({});
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setProfile(updated); setPainDraft({}); setStatus("Profile saved");
    ...
    <div className="form-page page-container">
      ...
      <form className="profile-form surface-panel" onSubmit={submit}>
        ...
        <div className="profile-lower-grid">
          <section className="form-section pain-editor"><div className="section-heading-title"><UserIcon size={21} /><div><h2>Pain range</h2><p className="muted">Rate each area from 0 (no pain) to {PAIN_MAX} (worst).</p></div></div><div className="pain-form-grid">{painAreas.map(({ key, label }) => <label key={key}><span>{label}</span><input id={key} aria-label={label} className="range-input" name={key} type="range" min="0" max={PAIN_MAX} defaultValue={profile[key]} onInput={(event) => { const value = Number(event.currentTarget.value); setPainDraft((current) => ({ ...current, [key]: value })); }} /><output htmlFor={key}>{painDraft[key] ?? profile[key]}</output></label>)}</div></section>
